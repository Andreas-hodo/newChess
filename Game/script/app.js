...
class Player{
  ...
  setCalcColor(){
    board.getSquares().forEach(square =>{
      ...
      let handleColors = (e) =>{
        let target = e.target
        let colors = {black: ["rgb(209,0,0)","black","rgb(100,75,43)"],white:["rgb(255,0,0)","white","rgb(240,201,150)"]}
        if(e.target.classList.contains("pieces") || !isDouble(e)) return   
        let isDefault = square.getColor() == colors[square.getDefaultColor()][1] ? true : false  
        let newColor = isDefault ? colors[square.getDefaultColor()][0] : board.getlast_move().includes(square.getName()) ? "rgb(227,224,150)" : colors[square.getDefaultColor()][2]
        isDefault || board.getlast_move().includes(square.getName()) ? square.applyColor(newColor) : square.removeColor()
      }
