...
class Player{
  ...
  drawArrow(x,y,length,type,direction,color,direction2 = direction){
  let canvas = document.querySelector("canvas") 
  let isBishop = type == "bishop" ? true : false
  let isKnight = type == "knight" ? true : false
  let coordX = x
  let coordY = y
  let directions = {up:[0,"y",2],down:[180,"y",-2],left:[270,"x",-2],right:[90,"x",2]}
  let context = canvas.getContext("2d")
  const squarewidth = 62
  const arrowH = squarewidth * (length - 1) - 31
  const arrowW = 15
  let finalX = squarewidth * (x) - squarewidth / 2 
  let finalY = squarewidth * (9 - y) - squarewidth / 2
  context.fillStyle = color
  context.strokeStyle = color
  let rotate = (direction) =>{
    let angle = directions[direction][0]
    let degrees = isBishop ? angle + 45 : angle
    const radiansangle = degrees * Math.PI / 180
    context.save()
    context.translate(finalX,finalY)
    context.rotate(radiansangle)
  }
  let drawArrowTail = (data) =>{
    let x = data.finalX
    let y = data.finalY    
    let height = isBishop ? data.arrowH + 26 * (length - 1) : isKnight ? data.arrowH + 31.5 * (length - 2) : data.arrowH 
    let width =  data.arrowW 
    context.fillRect(-width/2,-height - 7,width,height)
  }
  let drawTriangle = (data) =>{   
    let x = data.finalX
    let y = data.finalY   
    let width = data.arrowW 
    let height = isBishop ? data.arrowH + 26 * (length - 1) : isKnight ? data.arrowH + 31.5 * (length - 2) : data.arrowH 
    context.moveTo(0, -height - 30);
    context.lineTo(20,-height - 5);
    context.lineTo(-20,-height - 5);
    context.closePath();
  } 
  let updateCoord = (coord) =>{
    switch(coord){
        case "y":
            coordY = coordY + directions[direction][2]
            break
        case "x":
            coordX = coordX + directions[direction][2]
            break
    }
  }  
  rotate(direction) 
  context.beginPath(); 
  drawArrowTail({finalY,finalX,arrowH,arrowW})
  if(isKnight){
    context.restore()
    context.beginPath();
    updateCoord(directions[direction][1])
    this.drawArrow(coordX,coordY,2,"rook",direction2)
  }else{  
    context.beginPath(); 
    drawTriangle({finalY,finalX,arrowH,arrowW})  
  }
  context.fill()
  context.stroke();
  context.restore()
  game.updatedrawedArrows()
  }
...
class Piece extends Board{
  ...
  drag(piece,additionaMoves = []){
    ...
    moves.forEach(move =>{
      ...
      if(pieceobj.getType() == "pawn" && pieceobj.isPromotion(pieceColor.color,currentSquareY)){       
        currentSquare.applyColor("rgb(0,255,255)")
      } 
    }) 
  }
  drop(data,additionaMoves = []){
    ...
    if(pieceobj.getType() == "pawn"){  
      ...
      if(enpassant.isEnPassant) moves.push(enpassant.square)
    } 
    let castle = {canCastle:false}
    if(pieceobj.getType() == "king"){
      castle = pieceobj.Castle()
      let castleMoves = castle.moves
      pieceobj.TranferRook(castle,targetSquare.id)
      moves = moves.concat(castleMoves)
    } 
    moves = moves.concat(additionaMoves)
    ...
    pieces.forEach(piecename =>{
     ...
     for(let i = 0; i < piecemoves.length; i++){
      for(let j = 0; j < piecemoves[i].length; j++){
        ...
        pieceWasFound = true
        let EnemyDirection = piecemoves[i].slice(1,piecemoves[i].length)
        let backmoves = piecemoves[i].slice(0,piecemoves[i].indexOf(Squareid))
        for(let i = 0; i < EnemyDirection.length; i++){
          ...
          if(square.hasSpecificPiece(piecename,oppositeColor) || square.hasSpecificPiece("queen",oppositeColor)){
            isPinned = true
...
class King extends Piece{
  ...
  TranferRook(data,square){
    //Να ελεγχω εαν το canCastle ειναι true, εαν οχι να κανω return
   console.log(data)
  }
  Castle(){
   ...
   let data = {moves: [rook1.move, rook2.move].filter(Boolean),rook1,rook2};
   return data
  }
...
class Pawn extends Piece{
  ...
  OpenPromotion(y,target){
    ...
    let promotion = this.getPieceElement("promotion")
    game.setPromotionData({color:this.getColor(),square:this.getSquare(),x:this.getX(),y:this.getY(),targetSquare:target})
