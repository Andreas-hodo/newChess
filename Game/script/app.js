...
class Board extends Game{
 ...
 getstrFen(){
  ...
  Evaluation(){
    ...
    let createElems = () =>{   
      let players = [fplayer,splayer]
      let names = [fname,sname]
      let colors = ["white","black"]
      let data = []
      for(let i = 0; i < players.length; i++){        
        let currPlayer = document.createElement("span")
        currPlayer.id = `${colors[i]}material`
        currPlayer.classList.add("material")      
        data.push(currPlayer,names[i])
        if(players[i]) continue 
        names[i].appendChild(currPlayer)        
      }   
      return data     
    }
    [fplayer,fname,splayer,sname] = createElems()
    ...
    switch(materialCase){
      case 1:
        ...
        sname.removeChild(sname.firstElementChild)
        break
      case 2:
        fname.removeChild(fname.firstElementChild)
...
class Piece extends Board{
  ...
  drag(piece,additionaMoves = []){
    ...
    if(pieceobj.getType() == "pawn"){
      ...
      let enpCondition = (enpassant.isEnPassant && !isInCheck) || (enpassant.isEnPassant && isInCheck && CheckBlockMoves.includes(enpassant.square))
      if(enpCondition){
