...
class Pawn extends Piece{
  ...
  PinnedEnPassant(EnPassant,x,y,color,enemyColor){
    if(!EnPassant.isEnPassant) return   
    ...
    let checkSquares = (squares,piece,enemyColor) =>{
      for(let i = 0; i < squares.length; i++){
        let currentSquare = board.getSquare(squares[i])
        if(currentSquare.hasSpecificPiece("queen",enemyColor) || currentSquare.hasSpecificPiece("rook",enemyColor)) return true
        if(currentSquare.hasPiece()) return false
      }
    }
    let [empty,piece] = [checkSquares(map1,false,enemyColor),checkSquares(map2,true,enemyColor)]
  ...
  isEnPassant(color,x,y){
    ...
    this.PinnedEnPassant(result,x,enpassantY,color,enemyColor)
    return result;
  }
  Promote(type){
