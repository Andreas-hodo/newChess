...
class Piece extends Board{
  ...
  #drag(piece){
    ...
    // let opponentsKing = board.getPiece(`king_w`)
    // let opponentsKingStatus = opponentsKing.isInCheck(opponentsKing.getColor(),opponentsKing.getSquare(),opponentsKing.getX(),opponentsKing.getY())
    ...
    pieces.forEach(piecename =>{     
      ...
      for(let i = 0; i < cuttedmoves.length; i++){
        for(let j = 0; j < cuttedmoves[i].length; j++){
          ...
          console.log(checks,"Double check is not working, add pawn")
          if(checks == 2){
            return {isInCheck,blockmoves:[]}
          }         
          //console.log(piecename,square.getName(),hasPiece) 
        }
      }      
    })
    return {isInCheck,blockmoves}   
...
class Pawn extends Piece{
  ...
  concatMoves(){
    return {forward:this.moveForward(),diagonal:this.diagonalMove(1).concat(this.diagonalMove(-1))}
  }
  createMoves(){
    return Array.from(Object.values(this.concatMoves())).map((moves,i) => i == 0 ? this.cutDirection(moves) : this.cutDiagonal(moves)).flat()
  }
  getDirection(color){
