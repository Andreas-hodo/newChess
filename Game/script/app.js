...
class Piece extends Board{
  ...
  #drag(piece){
    ...
    this.isPinned(piece.id,pieceColor.color)  
     ...
     for(let i = 0; i < piecemoves.length; i++){
      for(let j = 0; j < piecemoves[i].length; j++){
        ...
        for(let i = 0; i < EnemyDirection.length; i++){
          ...
          let piecetype = board.getPiece(squarePiece).getType()
          if(square.hasSpecificPiece(piecename,oppositeColor) || square.hasSpecificPiece("queen",oppositeColor)){
            isPinned = true
            result = {isPinned,blockmoves:EnemyDirection.slice(0,i+1),backmoves:[]}
            console.log(piecetype,oppositeColor,square.getName(),"Legal moves: ",result.blockmoves)
            break
          }         
        }
      }
