...
class Piece extends Board{
  ...
  #drop(data){
    ...
    if(pieceobj.getType() == "pawn"){
     pieceobj.OpenPromotion(targetY) 
    }
    if(pieceobj.getType() == "pawn"){
    ...
    pieces.forEach(piecename =>{     
      ...
      for(let i = 0; i < cuttedmoves.length; i++){
        for(let j = 0; j < cuttedmoves[i].length; j++){
          ...
          let hasFriendlyKing = square.hasSpecificPiece(this.getType(),this.getColor())
          console.log("Friendly king: ",hasFriendlyKing," in ",squareId," square ")         
          if(!hasPiece) continue  
...
class Pawn extends Piece{
  ...
  OpenPromotion(){
    if(!this.isPromotion(this.getColor(),this.getY())) return
